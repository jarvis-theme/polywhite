      ...
      <div class="row">
              ...
              <ul class="unstyled pretty-list cl-effect-1">
                @foreach(recentBlog() as $recent)
                  <li>
                    ...
                    <br />
                    <small style="margin-left: 20px;">diposting {{waktuTgl($recent->updated_at)}}</small>
                  </li>
                @endforeach
              </ul>
            </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 section">
          <div class="cat-image"><h1 class="section-title">{{$detailblog->judul}}</h1></div>
              <small style="margin:8px;">Date: {{waktuTgl($detailblog->updated_at)}} <span>&nbsp;&nbsp; <i class="icon-tag"></i>&nbsp;<a href="{{URL::to('blog/category/'.$detailblog->kategori->nama)}}">{{$detailblog->kategori->nama}}</a></span></small>              
